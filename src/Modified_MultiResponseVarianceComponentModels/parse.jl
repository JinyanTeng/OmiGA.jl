...
"""
function h2(model::VCModel) # modified by Jinyan Teng in 20251117
    m, d = length(model.Σ), size(model.Σ[1], 1)
    h2s  = zeros(eltype(model.Y), m, d)
    ses  = zeros(eltype(model.Y), m, d)
    Σs  = zeros(eltype(model.Y), m, d) # added by Jinyan Teng
    Σses  = zeros(eltype(model.Y), m, d) # added by Jinyan Teng
    ...
    for j in 1:d
        for i in 1:m
            ...
            if i != m
                ...
                ses[i, j] = sqrt_nan(∇h2' * Σcov * ∇h2)
                Σs[i, j] = model.Σ[i][j, j] # added by Jinyan Teng
                Σses[i, j] = sqrt_nan(Σcov[i][j, j]) # added by Jinyan Teng
            else
