function cis_permutation_one_assoc!(permutation_method::String, gene_i::Int, exppheno::Vector{T}, Q::Union{Nothing,Matrix{T}}, cis_genotype::Matrix{T}, absr_perm::Vector{T}, _df_perm::DataFrame, _df_tops::DataFrame, _df_full::DataFrame, dof::Int; y_perms::Union{Nothing,Matrix{T}}=nothing, y_perms_res::Union{Nothing,Matrix{T}}=nothing, is_sample_byrow::Bool=true, fdr::T=T(0.05), variant_subset_index::Union{Nothing,AbstractVector{Int}}=nothing) where {T<:AbstractFloat}
    ...
    pval_g1_threshold = get_approx_p_from_r(sort(absr_perm; rev=true)[ceil(Int, n_perms * fdr)], dof)
...
function cis_permutation_one_assoc!(permutation_method::String, exppheno::Vector{T}, Q::Union{Nothing,Matrix{T}}, cis_genotype::Matrix{T}, absr_perm::Vector{T}, dof::Int; y_perms::Union{Nothing,Matrix{T}}=nothing, is_sample_byrow::Bool=true, fdr::T=T(0.05)) where {T<:AbstractFloat}
    n_cis_snps = size(cis_genotype, 2)
    n_perms = length(absr_perm)
    @timeit to "Perm 1.1" if permutation_method == "standard_fast"
        y_perms .= exppheno
    ...
    pval_g1_threshold = get_approx_p_from_r(sort(absr_perm; rev=true)[ceil(Int, n_perms * fdr)], dof)
    return pval_g1_threshold
end
function cis_permutation_one_assoc!(permutation_method::String, exppheno::Vector{T}, cis_genotype::Matrix{T}, absr_perm::Vector{T}, variant_subset_index::AbstractVector{Int}; Q::Union{Nothing,Matrix{T}}=nothing, y_perms::Union{Nothing,Matrix{T}}=nothing) where {T<:AbstractFloat}
    n_perms = length(absr_perm)
    @timeit to "Perm 1.1" if permutation_method == "standard_fast"
        y_perms .= exppheno
        Random.seed!(_args_seed)
...
function cis_permutation_two_assoc!(permutation_method::String, gene_i::Int, exppheno::Vector{T}, cis_genotype::Matrix{T}, cis_genotype_iterm::Matrix{T},
    ...
    pval_g2_threshold = get_approx_p_from_r(sort(max_absr_g2; rev=true)[ceil(Int, n_perms * fdr)], dof - 1) 
    @timeit to "Perm 1.2" begin
...
function cis_permutation_two_assoc!(permutation_method::String, exppheno::Vector{T}, cis_genotype::Matrix{T}, cis_genotype_iterm::Matrix{T},
    y_perms::Union{Nothing,Matrix{T}}, cissnps_annot::DataFrame, stat_perm::Vector{T};
    X_MME::Union{Nothing,Matrix{T}}=nothing, Xt_X::Union{Nothing,Matrix{T}}=nothing, glo_EA::Union{Nothing,Eigen{T,T,Matrix{T},Vector{T}}}=nothing,
    chrom_xtx_SQt_xQ::Union{Nothing,Matrix{T}}=nothing, chrom_xtx_SQ2t_xQ::Union{Nothing,Matrix{T}}=nothing, xW0::Union{Nothing,Matrix{T}}=nothing, xty_init::Union{Nothing,Matrix{T}}=nothing, xtx_init::Union{Nothing,Matrix{T}}=nothing, fdr::T=T(0.05)) where {T<:AbstractFloat}
    n_cis_snps = size(cissnps_annot, 1)
    n_perms = size(y_perms, 2)
    dof = size(X_MME, 1) - 2 - size(X_MME, 2)
    if permutation_method == "standard"
    ...
    pval_g2_threshold = get_approx_p_from_r(sort(max_absr_g2; rev=true)[ceil(Int, n_perms * fdr)], dof - 1) 
    return pval_g2_threshold
end
function cis_permutation_two_assoc!(permutation_method::String, exppheno::Vector{T}, cis_genotype_iterm::Matrix{T}, 
    y_perms::Union{Nothing,Matrix{T}}, X_MME::Union{Nothing,Matrix{T}}, Xt_X::Union{Nothing,Matrix{T}}, 
    variant_subset_index::AbstractVector{Int}) where {T<:AbstractFloat}
    n_perms = size(y_perms, 2)
    if permutation_method == "standard"
        error("!")
    elseif permutation_method == "standard_fast"  
