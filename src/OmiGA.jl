...
global relased_omiga_version = "1.5.0-beta.1+251118"
...
function real_main(ARGS)::Cint
    ...
    global parsed_args = get_parsed_args(ARGS)
    str_cmd = join(ARGS, " ")
    ...
    global _args_output_significant_qtls = parsed_args["output-significant-qtls"]
    ...
    global _args_low_mem = isnothing(_args_chunk_size) ? false : true
    ...
    println_to_file(replace(str_cmd, " --" => " \\\n--"), log_file)
    println_to_file("", log_file)
    ...
    elseif _args_run_mode == "enrich"
        @time runOmiGA_enrich(_args_enrich_target, _args_geno_file_prefix, _args_enrich_annot; maf_match=_args_maf_match, threshold=nothing, n_perms=_args_n_perms)
    end
    time_end_omiga = now()
